...
<script>
  ...
  import { scarpList } from "@/api/scarp";
  export default {
    data() {
      return {
        loading: {
    ...
    methods: {
      ...
      async onSelect(type) {
        ...
        var gmtApplyStart = form.applyTime[0];
        var gmtApplyEnd = form.applyTime[1];
        gmtStart = new Date(gmtStart).toLocaleDateString();
        gmtEnd = new Date(gmtEnd).toLocaleDateString();
        gmtApplyStart = new Date(gmtApplyStart).toLocaleDateString();
        gmtApplyEnd = new Date(gmtApplyEnd).toLocaleDateString();
        if (type === 1) {
          data.pageNum = 1;
        } else {
        ...
        data.gmtApplyStart = form.applyTime !== "" ? gmtApplyStart : "";
        data.gmtApplyEnd = form.applyTime !== "" ? gmtApplyEnd : "";
        var res = await scarpList(data);
